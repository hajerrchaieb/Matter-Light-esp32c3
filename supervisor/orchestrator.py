...
FUSION v6 + v7 :
  ...
  De v6 (conservé) :
    - Re-lecture de unit-test-results.json depuis le disque dans node_summary()
      (_ut_fresh) pour capturer les résultats Run 2 écrits après le démarrage
    - Fallback OTA : lecture de firmware-sha256.txt si ota-manifest vide
    - ota_rollout lu depuis le manifest (pas hardcodé à 10)
    - ota_note transmis dans pipeline-summary.json
    - Log OTA enrichi dans node_summary

  De v7 (ajouté) :
    - HIL BRANCHÉ : extraction complète des métriques HIL
      (hil_status, hil_tests, hil_passed_n, hil_failed_n,
       hil_device, hil_duration, hil_failures, hil_pass_rate)
    - Section "hil" complète dans pipeline-summary.json
    - HIL affiché dans le banner console et le PR comment
    - Log HIL détaillé dans node_fault_analysis

  Corrections supplémentaires (v8) :
    - \n superflu dans le tableau PR comment supprimé
    - Espace manquant dans _call_agent_with_retry(run_test_gen_agent, ...) corrigé
    - Commentaires inline harmonisés
"""
...
def node_summary(state: PipelineState) -> PipelineState:
    ...
    pr_lines += [
        "---",
        f"*Generated by {agents_run}/8 AI agents — "
