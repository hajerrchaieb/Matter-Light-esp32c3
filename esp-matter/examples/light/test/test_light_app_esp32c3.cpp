...
/* Test 5: stack overflow detection — robustness */
static esp_err_t mock_deep_recurse(int n) {
    if(n>100) return ESP_ERR_NO_MEM;
    return mock_deep_recurse(n+1);
}
void test_stack_overflow_detection(void) {
    esp_err_t ret = mock_deep_recurse(0);
