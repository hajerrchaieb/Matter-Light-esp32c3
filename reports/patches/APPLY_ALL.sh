...
echo "→ autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch"
if git apply --check "reports/patches/autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch" 2>/dev/null; then
  git apply "reports/patches/autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch"
  echo "  ✅ Appliqué"
  APPLIED=$((APPLIED+1))
elif git apply --check --ignore-whitespace "reports/patches/autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch" 2>/dev/null; then
  git apply --ignore-whitespace "reports/patches/autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch"
  echo "  ✅ Appliqué (ignore-whitespace)"
  APPLIED=$((APPLIED+1))
else
  echo "  ⚠️  Skip (autofix-esp32c3-09-security-tools_qemu_fault_runner.py.patch)"
  SKIPPED=$((SKIPPED+1))
fi
